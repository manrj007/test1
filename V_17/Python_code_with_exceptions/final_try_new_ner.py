# -*- coding: utf-8 -*-
"""
Created on Mon Apr  1 16:57:09 2019
...
# os.chdir('C:\\Users\\Karan.Singh.GISBIZDEV.000\\Desktop\\AI\\project\\Argus')
...
# trainingPath = "C:\\Users\\Karan.Singh.GISBIZDEV.000\\Desktop\\AI\\project\\Argus\\training\\"
# testingPath = "C:\\Users\\Karan.Singh.GISBIZDEV.000\\Desktop\\AI\\project\\Argus\\testing\\"
Flag = False
# doc1 = 'Model_SALES REPRESENTATIVE AGREEMENT_201903_3.txt'
# doc2 = 'SALES REPRESENTATIVE AGREEMENT_3.txt'
...
def get_documents(doc1, doc2 ):
    try:
        ...
        text_loaded = open(tempTest, encoding = check_encoding(tempTest)).read()
        search_sentence = open(tempTraining, encoding = check_encoding(tempTraining)).read()
                

        text_loaded = str(text_loaded).replace("\n", "").strip()
...
#                    if j.isalnum():
            ...
            # this is to write the output of pattern_matching_without_verb_entity(sentences, list(search_sentence_list[i]))
            # we aren't using it's output in BQM
            if not words_to_be_searched_stop_rem:
                pattern_matching_without_verb_entity(sentences, [search_sentence_list[i]])
            else:
                if (pattern_matching_verb_pos(sentences, [search_sentence_list[i]])[0] == [] and
                    pattern_matching_verb_entity(sentences, [search_sentence_list[i]])[0] == [] and
                    bqm_sent(sentences, [search_sentence_list[i]]) == [] and 
                    vsm_sent(sentences, [search_sentence_list[i]])[0] == []):
                    
                    Final_Sentences_Lenient = []
                else:
                    Final_Sentences_Lenient = [pattern_matching_verb_pos(sentences, [search_sentence_list[i]])[0] +
                                               pattern_matching_verb_entity(sentences, [search_sentence_list[i]])[0] +
#                                               bqm_sent(sentences, [search_sentence_list[i]]) +
                                               vsm_sent(sentences, [search_sentence_list[i]])[0]]
                    ...
                    with open('Uniq_Final_Sentences_Lenient.txt', 'a') as f:
                ...
                if (pattern_matching_verb_pos(sentences, [search_sentence_list[i]])[1] == [] and
                    pattern_matching_verb_entity(sentences, [search_sentence_list[i]])[1] == [] and
                    bqm_sent(sentences, [search_sentence_list[i]]) == [] and
                    vsm_sent(sentences, [search_sentence_list[i]])[1] == []):
                    
                    Final_Sentences_Moderate = []
                else:
                    Final_Sentences_Moderate = [pattern_matching_verb_pos(sentences, [search_sentence_list[i]])[1] + 
                                                pattern_matching_verb_entity(sentences, [search_sentence_list[i]])[1] +
#                                                bqm_sent(sentences, [search_sentence_list[i]]) + 
                                                vsm_sent(sentences, [search_sentence_list[i]])[1]]
                ...
                with open('Uniq_Final_Sentences_Moderate.txt', 'w') as f:
                    ...
                    # Strict Mode Output
                if (pattern_matching_verb_pos(sentences, [search_sentence_list[i]])[2] == [] and 
                        pattern_matching_verb_entity(sentences, [search_sentence_list[i]])[2] == [] and 
                            bqm_sent(sentences, [search_sentence_list[i]]) == [] and 
                                vsm_sent(sentences, [search_sentence_list[i]])[2] == []):
                    Final_Sentences_Strict = []
                else:    
                    Final_Sentences_Strict = [pattern_matching_verb_pos(sentences, [search_sentence_list[i]])[2] + 
                                              pattern_matching_verb_entity(sentences, [search_sentence_list[i]])[2] +
#                                              bqm_sent(sentences, [search_sentence_list[i]]) + 
                                              vsm_sent(sentences, [search_sentence_list[i]])[2]]
...
def pattern_matching_without_verb_entity(sentences, search_sentence_list_get_doc):

    
    
    print("*****************************************************************************************8 ")
    print("Pattern Matching Without Verb Entity Output")
    print(" ")
    output_words = []
    
    for i in range(len(search_sentence_list_get_doc)):           
        print(" ")
        print("Input of no verb   " + " " + str(i+1) + ":")
        print(search_sentence_list_get_doc[i])
        print(" ")
        input_entities = []
        user_input = search_sentence_list_get_doc[i]
        doc_input = nlp(user_input)
        print("Input Entities:    ")
        for j in doc_input.ents:
        ...
        for sent in unique_sentences:
            ...
            for k in doc_test.ents:
                ...
                if k.label_ in input_entities:
                    ...
                    final_words.append(k.text)
        
        with open('Uniq_Final_Sentences_Lenient.txt','a') as f:
            for s in final_words:
                print("The Entities found in the test document are :  ", s )
                f.write(s.strip() + '\n')

#get_documents(doc1, doc2 )       
##############################################################################
